...
fn main() {
    ...
    polygons.push(Rigidbody::rectangle(5.0, 1000.0, Vec2{x: -5.0, y: 0.0}));
    polygons[0].mass = f32::MAX / 10000000.0;
    polygons[0].calculate_moment_of_inertia();
    polygons[0].change_color(Color::new(1.0, 0.7, 0.1));

    polygons.push(Rigidbody::rectangle(0.5, 0.5, Vec2{x: 0.0, y: 0.0}));
    polygons[1].mass = 1.0;
    polygons[1].change_color(Color::blue());

    polygons.push(Rigidbody::rectangle(0.5, 0.5, Vec2{x: 5.0, y: 0.0}));
